...
const createTriggerPlugin = (options: TriggerPluginOptions = {}) =>
  createPlugin({
    id: 'trigger',
    ...
    prepare: async ({ tools, context, state, services }) => {
      ...
      tools.push(...createCurrentTriggerTools(triggerState.from.id, options.notifyHandler));
...
export { createTriggerPlugin, triggerPlugin };
