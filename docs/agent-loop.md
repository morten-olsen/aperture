...
graph TD
    ...
    GATHER_CONTEXT["gather_context()\nLoop through plugins\nRebuild system message\nUpdate state.system_context"]
    GATHER_CONTEXT --> RESOLVE_TOOLS

    RESOLVE_TOOLS["resolve_tools()\nCheck state.active_skills\nBuild tool list dynamically\nBind tools to agent node"]
    RESOLVE_TOOLS --> AGENT

    AGENT["agent()\nCall LLM with\nstate.messages +\nstate.system_context +\nresolved tools"]
    AGENT --> ROUTE_RESPONSE

    ROUTE_RESPONSE{"Route on\nresponse type"}
    ROUTE_RESPONSE -->|"text"| RECORD_TEXT["record_text()\nAppend to state.messages\nAppend to state.display_outputs"]
...
graph TD
    ...
    INVOKE["For each tool call:\n— Zod-validate input\n— Check requireApproval\n— Invoke or pause\n— Widgets/files emitted\n  as side-channel outputs"]
    INVOKE -->|"approval needed"| PAUSE(["Pause — wait for approve/reject"])
    INVOKE -->|"all done"| PREPARE

    PAUSE --> RESUME["Resume — replace pending result"]
    RESUME --> PREPARE
```
